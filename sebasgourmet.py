...
# --- SECCIÓN: GESTIÓN DE PEDIDOS ---
    ...
    if not pedidos:
        ...
        for i, p in enumerate(reversed(pedidos)):
            ...
            # Título del expander con soporte para iconos viejos y nuevos
            estado_display = p.get('estado', '🔴')
            # Asegurar compatibilidad con el formato anterior de JSON
            ...
            with st.expander(f"{estado_display} | {nombre_display} | Entrega: {entrega_display}"):
                ...
                # Mostrar items del formato nuevo o el tipo del formato viejo
                if "items" in p:
                    for item in p['items']:
                        st.write(f"- {item['producto']} x{item['cantidad']} {f'[{item['detalle']}]' if item['detalle'] else ''}")
                else:
                    # Lógica para mostrar datos del JSON antiguo
                ...
                # Manejo de iconos vs etiquetas de texto para el selector
                lista_est = list(estados_semaforo.keys())
                # Buscar si el estado guardado coincide con alguna etiqueta o icono
                est_idx = 0
